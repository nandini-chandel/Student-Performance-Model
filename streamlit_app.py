...
if not os.path.exists(MODEL_FILE):
    ...
    joblib.dump({"model": model_pipeline, "features": feature_columns}, MODEL_FILE)
    model = model_pipeline
else:
    obj = joblib.load(MODEL_FILE)
    model = obj["model"]
    feature_columns = obj["features"]

# =============================
# Sidebar Dropdown
# =============================
mode = st.sidebar.selectbox("Choose Mode", ["Home", "EDA", "Prediction"])

# =============================
# Home Mode
# =============================
if mode == "Home":
    st.subheader("👋 Welcome to Student Performance Predictions")
    st.markdown("""
    This app allows you to:
    
    - 📊 **Explore the dataset**: See trends, distributions, and correlations in student performance.  
    - 📈 **Predict final grades (G3)**: Input student information and get an estimated grade prediction.  

    The model is powered by a **Random Forest Regressor** trained on the  
    [UCI Student Performance Dataset](https://archive.ics.uci.edu/ml/datasets/student+performance).  

    ---
    🔹 Use the sidebar to switch between:
    - **EDA** → Explore the data  
    - **Prediction** → Try predicting a student’s final grade  
    """)
